...
class BoxedElement:
    ...
    def __init__(self, x, y, min_width, min_height):
...
def adjust_table_sizes_bak(row_hdr_btexts, col_hdr_btexts, bimgs):

    row_hdr_sizes = [[bt.get_box_width() for bt in lvl] \
                     for lvl in row_hdr_btexts]
    col_hdr_sizes = [[bt.get_box_width() for bt in lvl] \
                     for lvl in col_hdr_btexts]
    
    bimgs_sizes_1st_row = [bimg.get_box_width() for bimg in bimgs[0,:]]
    bimgs_sizes_1st_col = [bimg.get_box_height() for bimg in bimgs[:,0]]
    
    # Get adjusted sizes from column header:
    opt_row_item_sizes = cpt_opt_item_sizes_bak(row_hdr_sizes + \
                                                [bimgs_sizes_1st_col])
    opt_col_item_sizes = cpt_opt_item_sizes_bak(col_hdr_sizes + \
                                                [bimgs_sizes_1st_row])

    # Set adjusted sizes for headers:
    for sizes, hdr in ((opt_row_item_sizes, row_hdr_btexts),
                       (opt_col_item_sizes, col_hdr_btexts)):
        for item_size,btexts in zip(sizes, hdr):
            for btext in btexts:
                btext.set_box_width(item_size)

    # Set adjusted sizes for images:
    opt_img_w = opt_col_item_sizes[-1]
    opt_img_h = opt_row_item_sizes[-1]
    for row_bimgs in bimgs:
        for bimg in row_bimgs:
            bimg.set_box_width(opt_img_w)
            bimg.set_box_height(opt_img_h)            
...
def cpt_opt_item_sizes_bak(levels_sizes):
    """ Previous implementation with no spacers """
    max_size = 0.
    for level_sizes in levels_sizes:
        max_size = max(max_size, 1.*sum(level_sizes))

    item_sizes = []
    for level_sizes in levels_sizes:
        item_sizes.append(math.ceil(max_size / len(level_sizes)))
